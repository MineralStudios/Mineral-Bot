...
public class CrashReporter {
    public static void onCrashReport(Minecraft mc, CrashReport crashReport, CrashReportCategory category) {
        try {
            ...
            String reportStr = makeReport(mc, crashReport);
            ...
            FileUploadThread fut = new FileUploadThread(url, headers, content, listener, mc);
            fut.setPriority(10);
            fut.start();
            Thread.sleep(1000L);
    ...
    private static String makeReport(Minecraft mc, CrashReport crashReport) {
        StringBuffer sb = new StringBuffer();
        sb.append("OptiFineVersion: " + Config.getVersion() + "\n");
        sb.append("Summary: " + makeSummary(crashReport) + "\n");
        sb.append("\n");
        sb.append(crashReport.getCompleteReport(mc));
        sb.append("\n");
        return sb.toString();
    }
    ...
    private static String makeSummary(CrashReport crashReport) {
        ...
        if (t == null) {
            ...
            String sum = t.getClass().getName() + ": " + t.getMessage() + " (" + crashReport.getDescription() + ")"
                    + " [" + firstTrace + "]";
            return sum;
        }
    }
