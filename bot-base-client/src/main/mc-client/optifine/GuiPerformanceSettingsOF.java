...
public class GuiPerformanceSettingsOF extends GuiScreen {
    ...
    private static GameSettings.Options[] enumOptions = new GameSettings.Options[] { GameSettings.Options.SMOOTH_FPS,
            GameSettings.Options.SMOOTH_WORLD, GameSettings.Options.LOAD_FAR, GameSettings.Options.PRELOADED_CHUNKS,
            GameSettings.Options.CHUNK_UPDATES, GameSettings.Options.CHUNK_UPDATES_DYNAMIC,
            GameSettings.Options.FAST_MATH, GameSettings.Options.LAZY_CHUNK_LOADING, GameSettings.Options.FAST_RENDER };
    private int lastMouseX = 0;
    private int lastMouseY = 0;
    private long mouseStillTime = 0L;

    public GuiPerformanceSettingsOF(Minecraft mc, GuiScreen guiscreen, GameSettings gamesettings) {
        super(mc);
    ...
    public void initGui() {
        ...
        for (int k = 0; k < j; ++k) {
            ...
            if (!enumoptions.getEnumFloat()) {
                this.buttonList.add(new GuiOptionButton(this.mc, enumoptions.returnEnumOrdinal(), x, y, enumoptions,
                        this.settings.getKeyBinding(enumoptions)));
            } else {
                this.buttonList.add(new GuiOptionSlider(this.mc, enumoptions.returnEnumOrdinal(), x, y, enumoptions));
            }
    ...
    private String[] getTooltipLines(String btnName) {
        return btnName.equals("Smooth FPS")
                ? new String[] { "Stabilizes FPS by flushing the graphic driver buffers",
                        "  OFF - no stabilization, FPS may fluctuate", "  ON - FPS stabilization",
                        "This option is graphics driver dependant and its effect", "is not always visible" }
                : (btnName.equals("Smooth World")
                        ? new String[] { "Removes lag spikes caused by the internal server.",
                                "  OFF - no stabilization, FPS may fluctuate", "  ON - FPS stabilization",
                                "Stabilizes FPS by distributing the internal server load.",
                                "Effective only for local worlds and single-core CPU." }
                        : (btnName.equals("Load Far")
                                ? new String[] { "Loads the world chunks at distance Far.",
                                        "Switching the render distance does not cause all chunks ",
                                        "to be loaded again.", "  OFF - world chunks loaded up to render distance",
                                        "  ON - world chunks loaded at distance Far, allows",
                                        "       fast render distance switching" }
                                : (btnName.equals("Preloaded Chunks")
                                        ? new String[] { "Defines an area in which no chunks will be loaded",
                                        ...
                                        : (btnName.equals("Chunk Updates")
                                                ? new String[] { "Chunk updates",
                                                ...
                                                : (btnName.equals("Dynamic Updates") ? new String[] {
                                                        ...
                                                        : (btnName.equals("Lazy Chunk Loading") ? new String[] {
                                                                "Lazy Chunk Loading",
                                                                ...
                                                                "Effective only for local worlds and single-core CPU." }
                                                                : (btnName.equals("Fast Math") ? new String[] {
                                                                        "Fast Math", " OFF - standard math (default)",
                                                                        " ON - faster math",
                                                                        "Uses optimized sin() and cos() functions which can",
                                                                        "better utilize the CPU cache and increase the FPS." }
                                                                        : (btnName.equals("Fast Render")
                                                                                ? new String[] { "Fast Render",
                                                                                        ...
                                                                                        "Not compatible with shaders and some other mods!" }
                                                                                : null))))))));
    }
    ...
    private GuiButton getSelectedButton(int i, int j) {
        for (int k = 0; k < this.buttonList.size(); ++k) {
            ...
            boolean flag = i >= btn.field_146128_h && j >= btn.field_146129_i && i < btn.field_146128_h + btnWidth
