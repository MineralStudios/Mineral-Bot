...
public class WorldRendererThreaded extends WorldRenderer {
    ...
    public void updateRenderer(IWrUpdateListener updateListener) {
        if (this.worldObj != null) {
            ...
            byte one = 1;
            ChunkCache chunkcache = new ChunkCache(this.worldObj, xMin - one, yMin - one, zMin - one, xMax + one,
                    yMax + one, zMax + one, one);
    ...
    public void finishUpdate() {
        ...
        if (this.needsBoxUpdate && !this.skipAllRenderPasses()) {
            ...
            RenderItem.renderAABB(this.mc, AxisAlignedBB.getBoundingBox((double) this.posXClip, (double) this.posYClip,
