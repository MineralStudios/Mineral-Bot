...
public class CustomColorizer {
        ...
        for (int var20 = 0; var20 < var19.length; ++var20) {
            ...
            for (int ix = 0; ix < indexStrs.length; ++ix) {
                ...
                if (var21 >= 0 && var21 <= 255) {
                    if (var21 != Block.getIdFromBlock(Blocks.grass) && var21 != Block.getIdFromBlock(Blocks.tallgrass)
    ...
    public static int getColorMultiplier(Block block, IBlockAccess blockAccess, int x, int y, int z) {
        if (useDefaultColorMultiplier) {
            ...
            if (smoothColors1) {
                ...
                return colors != null ? getCustomColor(colors, blockAccess, x, y, z)
                        : block.colorMultiplier(blockAccess, x, y, z);
            }
        }
    }

    private static int getSmoothColorMultiplier(Block block, IBlockAccess blockAccess, int x, int y, int z,
            int[] colors, int[] swampColors, int type, int metadata) {
        int sumRed = 0;
        int sumGreen = 0;
        int sumBlue = 0;
        int r;
    ...
    public static int getSmoothColor(int[] colors, IBlockAccess blockAccess, double x, double y, double z, int samples,
            int step) {
        if (colors == null) {
            return -1;
        } else {
            ...
            if (worldType >= -1 && worldType <= 1) {
                ...
                if (lightMapRgb == null) {
                    ...
                    if (nightvision && height < 64) {
                        ...
                        if (width < 16) {
                            Config.warn("Invalid lightmap width: " + width + " for: /environment/lightmap" + worldType
                                    + ".png");
    ...
    public static Vec3 getWorldFogColor(Minecraft mc, Vec3 fogVec, WorldClient world, float partialTicks) {
        int worldType = world.provider.dimensionId;
        ...
        switch (worldType) {
            ...
            case 0:
                fogVec = getFogColor(fogVec, mc.theWorld, mc.renderViewEntity.posX, mc.renderViewEntity.posY + 1.0D,
    ...
    public static Vec3 getWorldSkyColor(Minecraft mc, Vec3 skyVec, WorldClient world, EntityLivingBase renderViewEntity,
            float partialTicks) {
        int worldType = world.provider.dimensionId;

        switch (worldType) {
            case 0:
                skyVec = getSkyColor(skyVec, mc.theWorld, mc.renderViewEntity.posX, mc.renderViewEntity.posY + 1.0D,
                        mc.renderViewEntity.posZ);
                break;
