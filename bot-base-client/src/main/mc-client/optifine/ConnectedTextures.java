...
public class ConnectedTextures {
    ...
    public static IIcon getConnectedTexture(Minecraft mc, IBlockAccess blockAccess, Block block, int x, int y, int z,
        ...
        if (blockAccess == null) {
            ...
            IIcon newIcon = getConnectedTextureSingle(mc, blockAccess, block, x, y, z, side, icon, true);
            ...
            if (!multipass) {
                ...
                for (int i = 0; i < 3; ++i) {
                    IIcon newMpIcon = getConnectedTextureSingle(mc, blockAccess, block, x, y, z, side, mpIcon, false);
    ...
    private static IIcon getConnectedTexture(ConnectedProperties cp, IBlockAccess blockAccess, Block block, int x,
        ...
        if (y >= cp.minHeight && y <= cp.maxHeight) {
            ...
            switch (cp.method) {
                ...
                case 8:
                    return getConnectedTextureHorizontalVertical(cp, blockAccess, block, x, y, z, var14, side, icon,
                            metadata);

                case 9:
                    return getConnectedTextureVerticalHorizontal(cp, blockAccess, block, x, y, z, var14, side, icon,
                            metadata);
    ...
    private static boolean isNeighbour(ConnectedProperties cp, IBlockAccess iblockaccess, Block block, int x, int y,
        ...
        if (cp.connect == 2) {
            ...
            return cp.connect == 3
                    ? (neighbourBlock == null ? false : neighbourBlock.getMaterial() == block.getMaterial())
                    : neighbourBlock == block && iblockaccess.getBlockMetadata(x, y, z) == metadata;
        }
    }
    ...
    private static IIcon getConnectedTextureVerticalHorizontal(ConnectedProperties cp, IBlockAccess blockAccess,
        ...
        if (iconV != null && iconV != icon && iconV != tileIcons[3]) {
            ...
            IIcon iconH = getConnectedTextureHorizontal(cp, blockAccess, block, x, y, z, vertAxis, side, icon,
                    metadata);
    ...
    private static boolean detectMultipass() {
        ...
        ConnectedProperties[] var6 = (ConnectedProperties[]) ((ConnectedProperties[]) propList
    ...
    private static ConnectedProperties[][] propertyListToArray(List list) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (subList != null) {
                ConnectedProperties[] subArr = (ConnectedProperties[]) ((ConnectedProperties[]) subList
