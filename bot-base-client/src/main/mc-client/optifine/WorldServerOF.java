...
public class WorldServerOF extends WorldServer {
    ...
    public List getPendingBlockUpdates(Chunk par1Chunk, boolean par2) {
        if (this.pendingTickListEntriesHashSet != null && this.pendingTickListEntriesTreeSet != null
                && this.pendingTickListEntriesThisTick != null) {
            ArrayList var3 = null;
                ...
                if (var9 == 0) {
                    ...
                    for (int dx = -1; dx <= 1; ++dx) {
                        for (int dz = -1; dz <= 1; ++dz) {
                            HashSet set = this.pendingTickListEntriesHashSet.getNextTickEntriesSet(var4.chunkXPos + dx,
                                    var4.chunkZPos + dz);
                            var11.addAll(set);
                        }
                    }
