...
public abstract class EntityLivingBase extends Entity implements ClientLivingEntity {
    ...
    public float prevSwingProgress, swingProgress, prevLimbSwingAmount, limbSwingAmount;

    /**
     * Only relevant when limbYaw is not 0(the entity is moving). Influences where
     * in its swing legs and arms currently
     * are.
     */
    ...
    /**
     * A factor used to determine how far this entity will move each tick if it is
     * walking on land. Adjusted by speed,
    ...
    protected void updateFallState(double p_70064_1_, boolean p_70064_3_) {
        ...
        if (p_70064_3_ && this.fallDistance > 0.0F) {
            ...
            if (var7.getMaterial() == Material.air) {
                ...
                this.worldObj.playAuxSFX(2006, var4, var5, var6,
    ...
    protected void onDeathUpdate() {
        ...
        if (this.deathTime == 20) {
            ...
            if (!this.worldObj.isClient && (this.recentlyHit > 0 || this.isPlayer()) && this.func_146066_aG()
    ...
    protected void updatePotionEffects() {
        Iterator<Entry<PotionEffect>> var1 = this.activePotionsMap.int2ObjectEntrySet().iterator();

        while (var1.hasNext()) {
            Entry<PotionEffect> e = var1.next();
            PotionEffect var3 = e.getValue();
        ...
        if (var11 > 0) {
            ...
            if (var4 && var11 > 0) {
                ...
                this.worldObj.spawnParticle(var12 ? "mobSpellAmbient" : "mobSpell",
                        this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width,
                        this.posY + this.rand.nextDouble() * (double) this.height - (double) this.yOffset,
                        this.posZ + (this.rand.nextDouble() - 0.5D) * (double) this.width, var5, var7, var9);
            }
        }
    }
    ...
    public void addPotionEffect(PotionEffect p_70690_1_) {
        if (this.isPotionApplicable(p_70690_1_)) {
            if (this.activePotionsMap.containsKey(p_70690_1_.getPotionID())) {
                ((PotionEffect) this.activePotionsMap.get(p_70690_1_.getPotionID()))
                        .combine(p_70690_1_);
                this.onChangedPotionEffect(
                        (PotionEffect) this.activePotionsMap.get(p_70690_1_.getPotionID()), true);
            } else {
    ...
    public void renderBrokenItemStack(ItemStack p_70669_1_) {
        ...
        for (int var2 = 0; var2 < 5; ++var2) {
            Vec3 var3 = Vec3.createVectorHelper(((double) this.rand.nextFloat() - 0.5D) * 0.1D,
                    Math.random() * 0.1D + 0.1D, 0.0D);
            var3.rotateAroundX(-this.rotationPitch * (float) Math.PI / 180.0F);
            var3.rotateAroundY(-this.rotationYaw * (float) Math.PI / 180.0F);
            Vec3 var4 = Vec3.createVectorHelper(((double) this.rand.nextFloat() - 0.5D) * 0.3D,
            ...
            this.worldObj.spawnParticle("iconcrack_" + Item.getIdFromItem(p_70669_1_.getItem()), var4.xCoord,
                    var4.yCoord, var4.zCoord, var3.xCoord, var3.yCoord + 0.05D, var3.zCoord);
        }
    }
     ...
     * {@link #swingProgress} to provide a
                ...
                if (var10 != 0 || var11 != 0) {
                    ...
                    if (this.worldObj.func_147461_a(var2).isEmpty()) {
                        if (World.doesBlockHaveSolidTopSurface(this.worldObj, var12, (int) this.posY, var13)) {
                            this.setPositionAndUpdate(this.posX + (double) var10, this.posY + 1.0D,
                                    this.posZ + (double) var11);
                            return;
                        }
    ...
    public void moveEntityWithHeading(float p_70612_1_, float p_70612_2_) {
        ...
        if (this.isInWater() && (!(this instanceof EntityPlayer) || !((EntityPlayer) this).capabilities.isFlying)) {
            ...
            if (this.isCollidedHorizontally && this.isOffsetPositionInLiquid(this.motionX,
                    this.motionY + 0.6000000238418579D - this.posY + var8, this.motionZ))
            ...
            if (this.onGround)
                var3 = this.worldObj.getBlock(MathHelper.floor_double(this.posX),
                        MathHelper.floor_double(this.boundingBox.minY) - 1,
                        MathHelper.floor_double(this.posZ)).slipperiness * 0.91F;

            float var4 = 0.16277136F / (var3 * var3 * var3);
            float var5 = this.onGround ? this.getAIMoveSpeed() * var4 : this.jumpMovementFactor;
            ...
            if (this.worldObj.isClient && (!this.worldObj.blockExists((int) this.posX, 0, (int) this.posZ)
            ...
            for (int var2 = 0; var2 < 5; ++var2) {
                ...
                if (!ItemStack.areItemStacksEqual(var4, var3)) {
                    ((WorldServer) this.worldObj).getEntityTracker().func_151247_a(this,
                            new S04PacketEntityEquipment(this.getEntityId(), var2, var4));
