...
public class DataWatcher {
    ...
    private static final Object2IntOpenHashMap<Class<?>> dataTypes;

    static {
        dataTypes = new Object2IntOpenHashMap<>();
        dataTypes.defaultReturnValue(-1);
    }

    private final Int2ObjectOpenHashMap<DataWatcher.WatchableObject> watchedObjects;
    ...
    public DataWatcher(Entity p_i45313_1_) {
        this.watchedObjects = new Int2ObjectOpenHashMap<>();
     ...
     * object see updateObject. Arguments: data
     * Value Id, Object to add
     */
    public void addObject(int key, Object val) {
        int var3 = dataTypes.getInt(val.getClass());

        if (var3 == -1)
            throw new IllegalArgumentException("Unknown data type: " + val.getClass());
        if (key > 31)
            throw new IllegalArgumentException("Data value id is too big with " + key + "! (Max is " + 31 + ")");
        if (this.watchedObjects.containsKey(key))
            throw new IllegalArgumentException("Duplicate id value for " + key + "!");

        DataWatcher.WatchableObject var4 = new DataWatcher.WatchableObject(var3, key, val);
        this.lock.writeLock().lock();
        this.watchedObjects.put(key, var4);
     ...
     */
    public void addObjectByDataType(int key, int dataType) {
        DataWatcher.WatchableObject var3 = new DataWatcher.WatchableObject(dataType, key, (Object) null);
        this.lock.writeLock().lock();
        this.watchedObjects.put(key, var3);
        this.lock.writeLock().unlock();
        this.isBlank = false;
    }
     ...
     */
    public int getWatchableObjectInt(int index) {
        Object obj = this.getWatchedObject(index).getObject();

        if (obj instanceof Integer i)
            return i.intValue();

        return obj instanceof Byte ? ((Byte) obj).byteValue() : obj instanceof Short ? ((Short) obj).shortValue() : 0;
    }
     ...
     * ChunkCoordinates}) to the specified PacketBuffer
     */
    private static void writeWatchableObjectToPacketBuffer(PacketBuffer p_151510_0_,
    ...
    /**
     * Reads a list of watched objects (entity attribute of type {byte, short, int,
     * float, string, ItemStack,
    ...
    public void updateWatchedObjectsFromList(List p_75687_1_) {
        ...
        while (var2.hasNext()) {
            DataWatcher.WatchableObject var3 = (DataWatcher.WatchableObject) var2.next();
            DataWatcher.WatchableObject var4 = this.watchedObjects
