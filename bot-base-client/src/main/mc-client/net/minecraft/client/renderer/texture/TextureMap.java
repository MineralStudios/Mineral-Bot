...
public class TextureMap extends AbstractTexture implements ITickableTextureObject, IIconRegister {
    ...
    public TextureMap(Minecraft mc, int par1, String par2Str) {
        this(mc, par1, par2Str, false);
    }

    public TextureMap(Minecraft mc, int par1, String par2Str, boolean skipFirst) {
        this.mc = mc;
        this.listAnimatedSprites = Lists.newArrayList();
        this.mapRegisteredSprites = Maps.newHashMap();
        this.mapUploadedSprites = Maps.newHashMap();
        this.field_147637_k = 1;
        this.missingImage = new TextureAtlasSprite(mc, "missingno");
        this.iconGrid = null;
        this.iconGridSize = -1;
        ...
        while (var5.hasNext() && !this.skipFirst) {
            ...
            if (var8.hasCustomLoader(par1ResourceManager, var25)) {
                ...
                try {
                    ...
                    if (var26 != null) {
                        ...
                        if (!var28.isEmpty()) {
                            ...
                            if (MathHelper.roundUpToPowerOfTwo(var18) != var18
                                    || MathHelper.roundUpToPowerOfTwo(var30) != var30) {
                    ...
                    logger.error(
                            "Using missing texture, unable to load " + sheetWidth + ", " + var23.getClass().getName());
            ...
            if (var2 == null) {
                if (this.textureType == 1) {
                    if ("clock".equals(par1Str)) {
                        var2 = new TextureClock(this.mc, par1Str);
                    } else if ("compass".equals(par1Str)) {
                        var2 = new TextureCompass(this.mc, par1Str);
                    } else {
                        var2 = new TextureAtlasSprite(this.mc, par1Str);
                    }
                } else {
                    var2 = new TextureAtlasSprite(this.mc, par1Str);
                }
    ...
    private void addDebugSprite(TextureAtlasSprite ts, BufferedImage image) {
        if (ts.getFrameCount() < 1) {
            ...
            image.setRGB(ts.getOriginX(), ts.getOriginY(), ts.getIconWidth(), ts.getIconHeight(), data, 0,
                    ts.getIconWidth());
        }
    }
