...
public class EntityRenderer implements IResourceManagerReloadListener {
     ...
     * Fog color buffer
     */
    transient FloatBuffer fogColorBuffer;
    /**
    ...
    public EntityRenderer(Minecraft mc, IResourceManager p_i45076_2_) {
            ...
            if (OpenGlHelper.shadersSupported) {
                ...
                if (this.shaderIndex != shaderCount) {
                    try {
                        ...
                        this.theShaderGroup = new ShaderGroup(this.mc, this.mc.getTextureManager(),
    ...
    public void onResourceManagerReload(IResourceManager par1ResourceManager) {
        ...
        if (this.shaderIndex != shaderCount) {
            try {
                this.theShaderGroup = new ShaderGroup(this.mc, this.mc.getTextureManager(), par1ResourceManager,
                        this.mc.getFramebuffer(), shaderResourceLocations[this.shaderIndex]);
                this.theShaderGroup.createBindFramebuffers(this.mc.displayWidth, this.mc.displayHeight);
    ...
    public void getMouseOver(float par1) {
        if (this.mc.renderViewEntity != null && this.mc.theWorld != null) {
            ...
            float var10 = 1.0F;
            @Nullable
            List var11 = this.mc.theWorld != null
                    ? this.mc.theWorld.getEntitiesWithinAABBExcludingEntity(this.mc.renderViewEntity,
    ...
    private void updateFovModifierHand() {
        if (this.mc.renderViewEntity instanceof EntityPlayerSP var1) {
            this.fovMultiplierTemp = var1.getFOVMultiplier();
        } else {
            this.fovMultiplierTemp = this.mc.thePlayer != null ? this.mc.thePlayer.getFOVMultiplier() : 1.0F;
        }
        ...
        if (this.debugViewDirection > 0) {
            ...
            if (this.mc.currentScreen == null) {
                if (this.mc.gameSettings.ofKeyBindZoom.getKeyCode() < 0) {
                    zoomActive = this.mc.getMouse().isButtonDown(this.mc.gameSettings.ofKeyBindZoom.getKeyCode() + 100);
                } else {
                    zoomActive = this.mc.getKeyboard().isKeyDown(this.mc.gameSettings.ofKeyBindZoom.getKeyCode());
                }
            }
    ...
    private void orientCamera(float par1) {
        ...
        if (var2.isPlayerSleeping()) {
            ...
            if (!this.mc.gameSettings.debugCamEnable) {
                @Nullable
                Block var27 = this.mc.theWorld != null ? this.mc.theWorld.getBlock(MathHelper.floor_double(var2.posX),
                        MathHelper.floor_double(var2.posY), MathHelper.floor_double(var2.posZ)) : null;

                if (Reflector.ForgeHooksClient_orientBedCamera.exists()) {
                    Reflector.callVoid(Reflector.ForgeHooksClient_orientBedCamera, this.mc, var2);
                } else if (var27 == Blocks.bed) {
                    int var11 = this.mc.theWorld != null
                            ? this.mc.theWorld.getBlockMetadata(MathHelper.floor_double(var2.posX),
                ...
                GL11.glRotatef(var2.prevRotationYaw + (var2.rotationYaw - var2.prevRotationYaw) * par1 + 180.0F, 0.0F,
                        -1.0F, 0.0F);
                GL11.glRotatef(var2.prevRotationPitch + (var2.rotationPitch - var2.prevRotationPitch) * par1, -1.0F,
                        0.0F, 0.0F);
            }
        } else if (this.mc.gameSettings.thirdPersonView > 0) {
            double var271 = this.thirdPersonDistanceTemp
                    + (this.thirdPersonDistance - this.thirdPersonDistanceTemp) * par1;
            float var28;
            float var281;
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                double var14 = (double) (-MathHelper.sin(var28 / 180.0F * (float) Math.PI)
                        * MathHelper.cos(var281 / 180.0F * (float) Math.PI)) * var271;
                ...
                for (int var20 = 0; var20 < 8; ++var20) {
                    ...
                    var23 *= 0.1F;
                    @Nullable
                    MovingObjectPosition var24 = this.mc.theWorld != null ? this.mc.theWorld.rayTraceBlocks(
        ...
        if (!this.mc.gameSettings.debugCamEnable) {
            GL11.glRotatef(var2.prevRotationPitch + (var2.rotationPitch - var2.prevRotationPitch) * par1, 1.0F, 0.0F,
                    0.0F);
            GL11.glRotatef(var2.prevRotationYaw + (var2.rotationYaw - var2.prevRotationYaw) * par1 + 180.0F, 0.0F, 1.0F,
                    0.0F);
        }
    ...
    private void setupCameraTransform(float par1, int par2) {
        ...
        Project.gluPerspective(this.getFOVModifier(par1, true),
                (float) this.mc.displayWidth / (float) this.mc.displayHeight, 0.05F, clipDistance);
        float var4;
        ...
        var4 = thePlayer != null ? thePlayer.prevTimeInPortal
                + (thePlayer.timeInPortal - thePlayer.prevTimeInPortal) * par1 : 0.0f;

        if (var4 > 0.0F) {
            byte var7 = 20;

            if (thePlayer != null && thePlayer.isPotionActive(Potion.confusion))
                var7 = 7;
    ...
    private void renderHand(float par1, int par2) {
        ...
        if (this.debugViewDirection <= 0) {
            ...
            Project.gluPerspective(this.getFOVModifier(par1, false),
            ...
            if (this.mc.gameSettings.thirdPersonView == 0 && !this.mc.renderViewEntity.isPlayerSleeping()
                    && !this.mc.gameSettings.hideGUI && !this.mc.playerController.enableEverythingIsScrewedUpMode()) {
    ...
    private void updateLightmap(Minecraft mc, float par1) {
        ...
        if (var2 != null) {
            ...
            for (int var3 = 0; var3 < 256; ++var3) {
                ...
                if (this.mc.thePlayer != null && this.mc.thePlayer.isPotionActive(Potion.nightVision)) {
    ...
    public void updateCameraAndRender(float par1) {
        ...
        if (!this.initialized && !BotGlobalConfig.optimizedGameLoop) {
            TextureUtils.registerResourceListener(this.mc);
        ...
        if (world1 != null && !BotGlobalConfig.optimizedGameLoop) {
            if (Config.getNewRelease() != null) {
                ...
                ChatComponentText var14 = new ChatComponentText(
                        "A new \u00a7eOptiFine\u00a7f version is available: \u00a7e" + var13 + "\u00a7f");
            ...
            if (Config.isNotify64BitJava()) {
                ...
                ChatComponentText var21 = new ChatComponentText(I18n
                        .format("You can install \u00a7e64-bit Java\u00a7f to increase performance"));
        ...
        if (!var22 && this.mc.gameSettings.pauseOnLostFocus
                && (!this.mc.gameSettings.touchscreen || !this.mc.getMouse().isButtonDown(1))) {
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            final int var161 = this.mc.getMouse().getX() * var142 / this.mc.displayWidth;
            final int var171 = var151 - this.mc.getMouse().getY() * var151 / this.mc.displayHeight - 1;
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ...
                    if (Reflector.EventBus_post.exists())
                        var12 = Reflector.postForgeBusEvent(Reflector.DrawScreenEvent_Pre_Constructor,
                                this.mc.currentScreen, Integer.valueOf(var161), Integer.valueOf(var171),
                                Float.valueOf(par1));

                    if (!var12 && this.mc.currentScreen != null)
                        this.mc.currentScreen.drawScreen(var161, var171, par1);

                    Reflector.postForgeBusEvent(Reflector.DrawScreenEvent_Post_Constructor,
                            this.mc.currentScreen, Integer.valueOf(var161), Integer.valueOf(var171),
                            Float.valueOf(par1));
                    ...
                    var11.addCrashSectionCallable("Screen name", () -> {
                        GuiScreen currentScreen = EntityRenderer.this.mc.currentScreen;
                        return currentScreen != null ? currentScreen.getClass().getCanonicalName() : "";
                    });
                    var11.addCrashSectionCallable("Mouse location",
                            () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d)",
                                    Integer.valueOf(var161), Integer.valueOf(var171),
                                    Integer.valueOf(EntityRenderer.this.mc.getMouse().getX()),
                                    Integer.valueOf(EntityRenderer.this.mc.getMouse().getY())));
                    var11.addCrashSectionCallable("Screen size",
                            () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d",
                                    Integer.valueOf(var133.getScaledWidth()),
                                    Integer.valueOf(var133.getScaledHeight()),
                                    Integer.valueOf(EntityRenderer.this.mc.displayWidth),
                                    Integer.valueOf(EntityRenderer.this.mc.displayHeight),
                                    Integer.valueOf(var133.getScaleFactor())));
    ...
    public void renderWorld(float par1, long par2) {
        ...
        for (int var13 = 0; var13 < 2; ++var13) {
            ...
            if (this.debugViewDirection == 0) {
                ...
                if (this.mc.objectMouseOver != null && var4.isInsideOfMaterial(Material.water)
                        && var4 instanceof EntityPlayer && !this.mc.gameSettings.hideGUI) {
            ...
            if (var5 != null)
                var5.drawBlockDamageTexture(this.mc.getTessellator(), var4, par1);
            GL11.glDisable(GL11.GL_BLEND);
            ...
            if (hasForge) {
                ...
                Reflector.callVoid(Reflector.ForgeHooksClient_dispatchRenderLast,
                        var5, Float.valueOf(par1));
            }

            this.mc.mcProfiler.endStartSection("hand");
            boolean renderFirstPersonHand = Reflector.callBoolean(Reflector.ForgeHooksClient_renderFirstPersonHand,
                    this.mc.renderGlobal, Float.valueOf(par1), Integer.valueOf(var13));
    ...
    private void addRainParticles() {
        ...
        if (var1 != 0.0F && mc.getConfig().isRainSplash()) {
            ...
            for (int var16 = 0; var16 < var15; ++var16) {
                ...
                int var19 = var3 != null ? var3.getPrecipitationHeight(var17, var18) : 0;
                @Nullable
                Block var20 = var3 != null ? var3.getBlock(var17, var19 - 1, var18) : null;
                @Nullable
                BiomeGenBase var21 = var3 != null ? var3.getBiomeGenForCoords(var17, var18) : null;

                if (var21 != null && var19 <= var5 + var7 && var19 >= var5 - var7 && var21.canSpawnLightningBolt()
                    ...
                    if (var20 != null && var20.getMaterial() == Material.lava) {
                        if (this.mc.effectRenderer != null)
                            this.mc.effectRenderer.addEffect(new EntitySmokeFX(var3, (float) var17 + var22,
                                    (double) ((float) var19 + 0.1F) - var20.getBlockBoundsMinY(),
                                    (float) var18 + var23, 0.0D, 0.0D, 0.0D));
            ...
            if (var14 > 0 && this.random.nextInt(3) < this.rainSoundCounter++) {
                ...
                if (var3 != null && var10 > var2.posY + 1.0D
                        && var3.getPrecipitationHeight(MathHelper.floor_double(var2.posX),
        ...
        if (Reflector.ForgeWorldProvider_getWeatherRenderer.exists()) {
            ...
            if (var41 != null) {
                Reflector.callVoid(var41, Reflector.IRenderHandler_render,
                        Float.valueOf(par1), this.mc.theWorld, this.mc);
                return;
            }
        }

        float var411 = this.mc.theWorld != null ? this.mc.theWorld.getRainStrength(par1) : 0.0f;
        ...
        if (var411 > 0.0F) {
            ...
            for (int var20 = var471 - var16; var20 <= var471 + var16; ++var20) {
                for (int var21 = var451 - var16; var21 <= var451 + var16; ++var21) {
                    ...
                    BiomeGenBase var25 = var441 != null ? var441.getBiomeGenForCoords(var21, var20) : null;

                    if (var25 != null && (var25.canSpawnLightningBolt() || var25.getEnableSnow())) {
                        int var26 = var441 != null ? var441.getPrecipitationHeight(var21, var20) : 0;
                        ...
                        if (var27 != var28) {
                            this.random.setSeed((long) var21 * var21 * 3121 + var21 * 45238971L
                                    ^ (long) var20 * var20 * 418711 + var20 * 13761L);
                            float var31 = var25.getFloatTemperature(var21, var27, var20);
                            float var32;
                            double var35;

                            if (var441 != null
                                    && var441.getWorldChunkManager().getTemperatureAtHeight(var31, var26) >= 0.15F) {
                                if (var18 != 0) {
                                    if (var18 >= 0 && var8 != null)
                                        var8.draw();
                                ...
                                var32 = ((float) (this.rendererUpdateCount + var21 * var21 * 3121 + var21 * 45238971
                                        + var20 * var20 * 418711 + var20 * 13761 & 31) + par1) / 32.0F
                                ...
                                if (var8 != null) {
                                    ...
                                    var8.setColorRGBA_F(var38, var38, var38,
                                            ((1.0F - var47 * var47) * 0.5F + 0.5F) * var411);
                                    var8.setTranslation(-var9, -var11, -var13);
                                    var8.addVertexWithUV((double) ((float) var21 - var23) + 0.5D, var27,
                                            (double) ((float) var20 - var24) + 0.5D, 0.0F * var29,
                                            (float) var27 * var29 / 4.0F + var32 * var29);
                                    var8.addVertexWithUV((double) ((float) var21 + var23) + 0.5D, var27,
                                            (double) ((float) var20 + var24) + 0.5D, var29,
                                            (float) var27 * var29 / 4.0F + var32 * var29);
                                    var8.addVertexWithUV((double) ((float) var21 + var23) + 0.5D, var28,
                                            (double) ((float) var20 + var24) + 0.5D, var29,
                                            (float) var28 * var29 / 4.0F + var32 * var29);
                                    var8.addVertexWithUV((double) ((float) var21 - var23) + 0.5D, var28,
                                            (double) ((float) var20 - var24) + 0.5D, 0.0F * var29,
                                            (float) var28 * var29 / 4.0F + var32 * var29);
                                ...
                                float var48 = this.random.nextFloat()
                                ...
                                if (var8 != null) {
                                    if (var441 != null)
                                        var8.setBrightness(
                                                (var441.getLightBrightnessForSkyBlocks(var21, var30, var20, 0) * 3
                                                        + 15728880)
                                                        / 4);
                                    var8.setColorRGBA_F(var40, var40, var40,
                                            ((1.0F - var39 * var39) * 0.3F + 0.5F) * var411);
                                    var8.setTranslation(-var9, -var11, -var13);
                                    var8.addVertexWithUV((double) ((float) var21 - var23) + 0.5D, var27,
                                            (double) ((float) var20 - var24) + 0.5D, 0.0F * var29 + var48,
                                            (float) var27 * var29 / 4.0F + var32 * var29 + var34);
                                    var8.addVertexWithUV((double) ((float) var21 + var23) + 0.5D, var27,
                                            (double) ((float) var20 + var24) + 0.5D, var29 + var48,
                                            (float) var27 * var29 / 4.0F + var32 * var29 + var34);
                                    var8.addVertexWithUV((double) ((float) var21 + var23) + 0.5D, var28,
                                            (double) ((float) var20 + var24) + 0.5D, var29 + var48,
                                            (float) var28 * var29 / 4.0F + var32 * var29 + var34);
                                    var8.addVertexWithUV((double) ((float) var21 - var23) + 0.5D, var28,
                                            (double) ((float) var20 - var24) + 0.5D, 0.0F * var29 + var48,
                                            (float) var28 * var29 / 4.0F + var32 * var29 + var34);
                                    var8.setTranslation(0.0D, 0.0D, 0.0D);
    ...
    private void updateFogColor(float par1) {
        ...
        @Nullable
        Vec3 var5 = var2 != null ? var2.getSkyColor(this.mc.renderViewEntity, par1) : null;
        var5 = CustomColorizer.getWorldSkyColor(this.mc, var5, var2, this.mc.renderViewEntity, par1);
        float var6 = var5 != null ? (float) var5.xCoord : 0.0f;
        float var7 = var5 != null ? (float) var5.yCoord : 0.0f;
        float var8 = var5 != null ? (float) var5.zCoord : 0.0f;
        @Nullable
        Vec3 var9 = var2 != null ? var2.getFogColor(par1) : null;
        var9 = CustomColorizer.getWorldFogColor(this.mc, var9, var2, par1);
        this.fogColorRed = var9 != null ? (float) var9.xCoord : 0.0f;
        this.fogColorGreen = var9 != null ? (float) var9.yCoord : 0.0f;
        this.fogColorBlue = var9 != null ? (float) var9.zCoord : 0.0f;
        float var11;

        if (this.mc.gameSettings.renderDistanceChunks >= 4) {
            @Nullable
            Vec3 var19 = var2 != null ? MathHelper.sin(var2.getCelestialAngleRadians(par1)) > 0.0F
                    ? Vec3.createVectorHelper(-1.0D, 0.0D, 0.0D)
                    : Vec3.createVectorHelper(1.0D, 0.0D, 0.0D) : null;
            var11 = var19 != null ? (float) var3.getLook(par1).dotProduct(var19) : 0.0f;

            if (var11 < 0.0F)
                var11 = 0.0F;

            if (var11 > 0.0F) {
                float[] var20 = var2 != null ? var2.provider.calcSunriseSunsetColors(var2.getCelestialAngle(par1), par1)
                        : new float[0];
        ...
        float var191 = var2 != null ? var2.getRainStrength(par1) : 0.0f;
        float var201;
        ...
        var11 = var2 != null ? var2.getWeightedThunderStrength(par1) : 0.0f;
        ...
        if (this.cloudFog) {
            fogYFactor = var2 != null ? var2.getCloudColour(par1) : null;
            this.fogColorRed = fogYFactor != null ? (float) fogYFactor.xCoord : 0.0f;
            this.fogColorGreen = fogYFactor != null ? (float) fogYFactor.yCoord : 0.0f;
            this.fogColorBlue = fogYFactor != null ? (float) fogYFactor.zCoord : 0.0f;
            ...
            fogYFactor = CustomColorizer.getUnderwaterColor(this.mc.theWorld, this.mc.renderViewEntity.posX,
        ...
        if (Reflector.EntityViewRenderEvent_FogColors_Constructor.exists()) {
            Object event1 = Reflector.newInstance(Reflector.EntityViewRenderEvent_FogColors_Constructor,
                    this, var3, var21, Float.valueOf(par1), Float.valueOf(this.fogColorRed),
                    Float.valueOf(this.fogColorGreen), Float.valueOf(this.fogColorBlue));
            Reflector.postForgeBusEvent(event1);
            this.fogColorRed = Reflector.getFieldValueFloat(event1, Reflector.EntityViewRenderEvent_FogColors_red,
                    this.fogColorRed);
            this.fogColorGreen = Reflector.getFieldValueFloat(event1, Reflector.EntityViewRenderEvent_FogColors_green,
                    this.fogColorGreen);
            this.fogColorBlue = Reflector.getFieldValueFloat(event1, Reflector.EntityViewRenderEvent_FogColors_blue,
                    this.fogColorBlue);
        }
        ...
        if (par1 == 999) {
            ...
            if (Reflector.postForgeBusEvent(event)) {
                ...
                if (var3.isPotionActive(Potion.blindness)) {
                    ...
                    if (this.mc.theWorld != null
                            && this.mc.theWorld.provider.doesXZShowFog((int) var3.posX, (int) var3.posZ)) {
                        var6 = this.farPlaneDistance;
                        GL11.glFogf(GL11.GL_FOG_START, var6 * 0.05F);
                        GL11.glFogf(GL11.GL_FOG_END, var6);
                    }

                    Reflector.postForgeBusEvent(Reflector.newInstance(
                            Reflector.EntityViewRenderEvent_RenderFogEvent_Constructor, this, var3, var5,
                            Float.valueOf(par2), Integer.valueOf(par1), Float.valueOf(var6)));
                }
            }
    ...
    private void showLagometer(long tickTimeNano, long chunkTimeNano) {
        if (this.mc.gameSettings.ofLagometer || this.showExtendedDebugInfo) {
            ...
            for (int frameNum = 0; frameNum < this.frameTimes.length; ++frameNum) {
                int lum = (frameNum - this.numRecordedFrameTimes & this.frameTimes.length - 1) * 255
                ...
                tessellator.addVertex((float) frameNum + 0.5F,
                        baseHeight - (float) heightFrame + 0.5F, 0.0D);
                ...
                tessellator.addVertex((float) frameNum + 0.5F, baseHeight + 0.5F, 0.0D);
                tessellator.addVertex((float) frameNum + 0.5F,
                        baseHeight + (float) heightTick + 0.5F, 0.0D);
                ...
                tessellator.addVertex((float) frameNum + 0.5F, baseHeight + 0.5F, 0.0D);
                tessellator.addVertex((float) frameNum + 0.5F,
                        baseHeight + (float) heightChunk + 0.5F, 0.0D);
                ...
                if (srvTime > 0L) {
                    ...
                    tessellator.addVertex((float) frameNum + 0.5F, baseHeight + 0.5F, 0.0D);
                    tessellator.addVertex((float) frameNum + 0.5F,
                            baseHeight + (float) heightSrv + 0.5F, 0.0D);
                }
            }
