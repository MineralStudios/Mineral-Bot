...
public class MathHelper {
     ...
     * of this array) is the upper 5 bits
     * of the result of multiplying a 32-bit unsigned integer by the B(2, 5) De
     * Bruijn sequence 0x077CB531. Value
     * (value stored in the array) is the unique index (from the right) of the
     * leftmost one-bit in a 32-bit unsigned
     * integer that can cause the upper 5 bits to get that value. Used for highly
     * optimized "find the log-base-2 of
    ...
    public static final float sin(float par0) {
        return fastMath ? SIN_TABLE_FAST[(int) (par0 * 651.8986F) & 4095]
                : SIN_TABLE[(int) (par0 * 10430.378F) & 65535];
    }
    ...
    public static final float cos(float par0) {
        return fastMath ? SIN_TABLE_FAST[(int) ((par0 + ((float) Math.PI / 2F)) * 651.8986F) & 4095]
                : SIN_TABLE[(int) (par0 * 10430.378F + 16384.0F) & 65535];
    }
     ...
     * upper limits given by the second and
     * third parameters.
     */
     ...
     * the log-base-two of the given
     * value. Optimized for cases where the input value is a power-of-two. If the
     * input value is not a power-of-two,
     ...
     * is effectively the index of the
     * highest bit that is set. For example, if the number in binary is 0...100101,
     * this will return 5.
     */
